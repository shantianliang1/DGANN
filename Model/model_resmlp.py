...
class Gnn(nn.Module):
    def __init__(self, ks, in_dim, num_classes, num_queries):
        ...
        self.start_resmlp = ResMLP(in_dim)

        # Down sampling layers with ResMLP
        for l in range(l_n):
            ...
            self.add_module(f'down_mlp_{l}', down_mlp)
            self.add_module(f'down_gcn_{l}', down_gcn)
            self.add_module(f'up_mlp_{l}', up_mlp)
            self.add_module(f'up_gcn_{l}', up_gcn)
            self.add_module(f'pool_{l}', pool)
            self.add_module(f'unpool_{l}', unpool)
            self.add_module(f'down_resmlp_{l}', ResMLP(in_dim))
            self.add_module(f'up_resmlp_{l}', ResMLP(in_dim))

        # Bottom layers with ResMLP
        ...
        self.bottom_resmlp = ResMLP(in_dim)
    ...
    def forward(self, A_init, X):
        ...
        X = self.start_resmlp(X)

        for i in range(self.l_n):
            A_old = A_new
            A_new = self._modules[f'down_mlp_{i}'](X)
            X = self._modules[f'down_gcn_{i}'](A_new, A_old, X)
            X = self._modules[f'down_resmlp_{i}'](X)

            adj_ms.append(A_new)
            down_outs.append(X)
            A_new, X, idx_batch = self._modules[f'pool_{i}'](A_new, X)
            indices_list.append(idx_batch)

        A_old = A_new
        A_new = self._modules['bottom_mlp'](X)
        X = self._modules['bottom_gcn'](A_new, A_old, X)
        X = self.bottom_resmlp(X)

        for i in range(self.l_n):
            up_idx = self.l_n - i - 1
            A_old, idx_batch = adj_ms[up_idx], indices_list[up_idx]
            A_old, X = self._modules[f'unpool_{i}'](A_old, X, idx_batch)
            X = X.add(down_outs[up_idx])

            X = self._modules[f'up_resmlp_{i}'](X)

            A_new = self._modules[f'up_mlp_{up_idx}'](X)
            X = self._modules[f'up_gcn_{up_idx}'](A_new, A_old, X)
...
class ResMLP(nn.Module):
    def __init__(self, dim, layerscale_init=1e-6):
    ...
    def forward(self, x):
        res = self.mlp(self.affine(x))
        return x + self.layerscale * res
